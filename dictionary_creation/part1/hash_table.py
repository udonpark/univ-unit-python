...
class LinearProbeHashTable(Generic[T]):
    ...
    def __linear_probe(self, key: str, is_insert: bool) -> int:
        ...
        _probe_max = 0 # newly added variable
        has_conflicted = False # newly added as a modification
        ...
        for _ in range(len(self.table)):  # start traversing
            if self.table[position] is None:  # found empty slot
                ...
                self.probe_total += 1
                _probe_max += 1
                if _probe_max > self.probe_max:
                    self.probe_max = _probe_max
                position = (position + 1) % len(self.table)
    ...
    def statistics(self) -> Tuple:
        """
        A method that returns tuple with 4 elements:
            conflict_count: the total number of conflicts,
            proble_total: the sum of all the probe chain lengths,
            proble_max: the length of the longest probe chain,
            rehash_count: the number of times rehash has been called.
        I have added accordingly, new instance variables in the constructor for this class.
        Furthermore, main modification has been done in __linear_probe method in LinearProbeHashTable.
        I don't have to reassign each to another variable, but just for the easier visualization
        :complexity: O(1), a simple return of tuple
        """
